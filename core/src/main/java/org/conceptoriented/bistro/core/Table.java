...
public class Table {
    ...
    //
    // Operations with multiple column columnPaths
    //
    ...
    public long find(List<Column> columns, List<Object> values, boolean append) {
        ...
        //List<Object> columnPaths = names.stream().map(x -> record.get(x)).collect(Collectors.<Object>toList());
		...
		for(long i=searchRange.start; i<searchRange.end; i++) { // Scan all records and compare
			...
			for(int j=0; j<columns.size(); j++) {
				...
				//   *SOLUTION 2: assume that the columnPaths have the type of the column, that is, the same comparable numeric type
